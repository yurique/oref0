...
function reduce(treatments) {
    ...
    function within_minutes_from(origin, tail, minutes) {
        ...
        return /* candidates */ tail.slice().filter(elem => {
    ...
    function bolus(ev, remaining) {
        if (!ev) {
            console.error('XXX', ev, remaining)
            return
        }
        ...
        if (remaining && remaining.length > 0) {
            ...
            state.insulin =
                ...
                (state.square ? state.square.amount : 0) +
                (state.bolus ? state.bolus.amount : 0)
            const has_insulin = state.insulin && state.insulin > 0
            const has_carbs = state.carbs && state.carbs > 0
            if (state.square && state.bolus) {
            ...
            if (has_insulin) {
                ...
                if (fs.existsSync(iobFile)) {
                    const iob = JSON.parse(fs.readFileSync(iobFile))
            ...
            if (state.wizard) {
                ...
                const delta = state.wizard.sensitivity * state.insulin * -1
