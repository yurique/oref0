...
function tuneAllTheThings (inputs: any) {
    ...
    CRData.forEach(function(CRDatum: any) {
    ...
    var autotuneMin: number = 0.7
    var autotuneMax: number = 1.2
    ...
    let deviationsArray: number[] = [];
    var BGIs = [];
    var avgDeltas = [];
    var ratios = [];
    for (i=0; i < ISFGlucose.length; ++i) {
        var deviation = parseFloat(ISFGlucose[i].deviation);
    ...
    avgDeltas.sort(function(a: number, b: number){return a-b});
    BGIs.sort(function(a: number, b: number){return a-b});
    deviationsArray.sort(function(a: number, b: number){return a-b});
    ratios.sort(function(a: number, b: number){return a-b});
    var p50deviation = percentile(deviationsArray, 0.50);
