...
export default function findMealInputs (inputs: Input): MealTreatment[] {
    ...
    let mealInputs: TempMealTreatment[] = [];
    var bolusWizardInputs: PumpHistoryEvent[] = [];

    const timestampEq = fromEquals((a: string, b: string) => Math.abs(new Date(a).getTime() - new Date(b).getTime()) < 2000)

    for (var i=0; i < carbHistory.length; i++) {
        const current = carbHistory[i];
        if (current.carbs && current.created_at) {
            mealInputs.push(createMeal(
                current.created_at,
                {
    ...
    for (i=0; i < pumpHistory.length; i++) {
        ...
        } else if (NightscoutTreatment.is(current) && current.created_at && (current.eventType === "Meal Bolus" || current.eventType === "Correction Bolus" || current.eventType === "Snack Bolus" || current.eventType === "Bolus Wizard" || current.eventType === "Carb Correction")) {
        ...
        } else if (PumpHistoryEvent.is(current) && current._type === "JournalEntryMealMarker" && current.carb_input && current.carb_input > 0) {
            mealInputs.push(createMeal(
                current.timestamp,
                {
    ...
    return uniq<MealTreatment>(eq)(mealInputs).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime())
}
