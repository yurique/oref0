...
// 5.5 mmol/L = 99.1 mg/dL, which rounds to 99. Against a threshold of 100 a 5.5 temp target reads
// as a *low* temp target, so low_temptarget_lowers_sensitivity would raise basal for what is really
// a normal target. 99 keeps mg/dL and mmol/L users consistent.
// AndroidAPS Constants.NORMAL_TARGET_MGDL (3ebb4b17a1).
var NORMAL_TARGET_MGDL = 99;
...
function enable_smb(
    ...
    if (! microBolusAllowed) {
        ...
        return false;
    } else if (! profile.allowSMB_with_high_temptarget && profile.temptargetSet && target_bg > NORMAL_TARGET_MGDL) {
...
// Calculate sensitivityRatio based on temp targets, if applicable, or using the value calculated by autosens
    ...
    var normalTarget = NORMAL_TARGET_MGDL; // evaluate high/low temptarget against the normal target, not the scheduled target (which might change)
...
// and before returning (doing nothing) below if eventualBG is undefined.
    ...
    rT.temp = 'absolute';
    rT.deliverAt = deliverAt;
    if ( microBolusAllowed && currenttemp && typeof currenttemp.rate === 'number' && iob_data.lastTemp && typeof iob_data.lastTemp.rate === 'number' && Math.abs(currenttemp.rate - iob_data.lastTemp.rate) > 0.001 && lastTempAge > 10 && currenttemp.duration ) {
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            // seconds since last bolus. Kept in seconds rather than minutes: the loop does not
            // fire on exact interval boundaries, so a bolus 2.99m ago against a 3m interval used to
            // fail this test and block the SMB for another whole loop cycle.
            var lastBolusAge = ( new Date(systemTime).getTime() - iob_data.lastBolusTime ) / 1000;
            ...
            SMBInterval = SMBInterval * 60;   // in seconds
            //console.error(naive_eventualBG, insulinReq, worstCaseInsulinReq, durationReq);
            console.error("naive_eventualBG " + convert_bg(naive_eventualBG,profile)  +", " + durationReq + "m " + smbLowTempReq + "U/h temp needed; last bolus " + round(lastBolusAge/60,1) + "m ago; maxBolus: "+maxBolus);
            ...
            if (lastBolusAge > SMBInterval - 6) {   // 6s tolerance
                ...
                var nextBolusMins = round((SMBInterval - lastBolusAge)/60 - nextBolusSeconds/60, 0);
